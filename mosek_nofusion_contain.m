function [C, d] = mosek_nofusion_contain(A, b, pp, C0, d_lb, d_ub)
...
C0_inv=inv(C0);
nvar = 0;
for v = {'t1', 't2', 'd', 'h', 's', 'sprime', 'z', 'f', 'g'}
  var = v{1};
  ndx.(var) = nvar + (1:num.(var));
  nvar = nvar + num.(var);
end
ndx.f = reshape(ndx.f, m, n);
ndx.h = reshape(ndx.h, c, n);

ncon = 1+n * m + c * n + m + n + n + (2^l - n) + 1 + (n * (n-1) / 2) + (2^l - 1);
...
for i=1:c
    rhs=C0_inv*pp(i,:)';
    for j=1:n
        prob.a(con_ndx,ndx.d(1:n))=C0_inv(j,:);
        prob.a(con_ndx,ndx.h(i,j))=1;
        prob.blc(con_ndx)=rhs(j);
        prob.buc(con_ndx)=rhs(j);
        con_ndx = con_ndx+1;
    end
end

prob.a(con_ndx,ndx.t1)=1;
prob.blc(con_ndx)=0.1;
prob.buc(con_ndx)=inf;
con_ndx = con_ndx+1;
...
for i=1:n
    prob.a(con_ndx,ndx.d(i))=1;
    prob.blc(con_ndx) = d_lb(i);
    prob.buc(con_ndx) = d_ub(i);
    con_ndx=con_ndx+1;
end
cone_ptr = 1;
...
for i=1:c
    prob.cones.type = [prob.cones.type, MSK_CT_QUAD];
    prob.cones.sub = [prob.cones.sub, ndx.t2(i), ndx.h(i,:)];
    prob.cones.subptr = [prob.cones.subptr, cone_ptr];
    cone_ptr = cone_ptr + n + 1;
end
...
if DEBUG
  ...
  t1 = res.sol.itr.xx(ndx.t1);
  t2 = res.sol.itr.xx(ndx.t2);
  ...
  assert(abs(2^(l/2) * t1 - s(end)) < 1e-4);
